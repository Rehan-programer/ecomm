...
import { addToFavourite, removeFromFavourite } from "@/redux/slice/favouriteslice";
...
console.log("products" ,product );
  ...
  const filter = product?.find((p) => String(p.id) === womenid);

  if (!filter) return <div className="p-8 text-gray-500">Loading...</div>;

  const isFav = favourites.some((fav) => fav.id === product.id);

  const toggleFavourite = () => {
    const exists = favourites.find((fav) => fav.id === product.id);
    exists
    ...
    <div className="p-8 bg-white rounded-2xl shadow-xl max-w-5xl mx-auto flex flex-col md:flex-row gap-8">
      <div className="relative flex-shrink-0">
        <button
          ...
          className={`absolute top-3 right-3 z-10 p-2 rounded-full border shadow-md transition ${
            isFav
      ...
      <div className="flex flex-col justify-between space-y-4 flex-1">
        ...
        <div className="flex gap-4 pt-4">
          ...
          >
            <Heart size={18} />{" "}
            {isFav ? "Remove Wishlist" : "Add to Wishlist"}
          </button>
        </div>
      </div>
    </div>
