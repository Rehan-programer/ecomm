...
export default function BabyProductDetails({ params }) {
  const { babyid } = React.use(params);
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/products?id=${babyid}`);
   ...
   const filter = product?.find((p) => String(p.id) === babyid);
