...
function DashboardWithRedux({ children }) {
  ...
  return (
    <div className="flex">
      <div className={`h-screen fixed left-0 top-0 bg-white shadow transition-all duration-300 ease-in-out ${sidebarWidth}`}>
        <Sidebar open={isSidebarOpen} toggleSidebar={toggleSidebar} />
      </div>

      <div className={`flex-1 transition-all duration-300 ease-in-out ${marginLeft}`}>
        <header className={`h-16 fixed top-0 right-0 left-0 bg-white p-4 shadow flex justify-between items-center z-10 transition-all duration-300 ease-in-out ${marginLeft}`}>
          <div className="flex items-center gap-4">
            ...
            <span className="text-gray-600 font-medium">Search / Breadcrumb</span>
          </div>
